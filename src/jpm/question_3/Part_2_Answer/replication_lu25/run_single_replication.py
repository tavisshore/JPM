...
def save_results_report(results, filename):
    """Save  report"""
    with open(filename, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write(f"SINGLE REPLICATION TEST REPORT\n")
        ...
        # Results by method
        for method_name in ["BLP + Cost IV", "BLP - Cost IV", "Shrinkage", "Lu25 MAP (No IV, No Inversion)"]:
            ...
            if est["status"] == "success":
                f.write(f"{'Parameter':<12} {'Estimate':>12} {'True Value':>12} {'Bias':>12} {'Assessment':>15}\n")
                f.write("-" * 80 + "\n")
                
                # sigma
                bias_sigma = est["sigma"] - true["sigma"]
                assess = "Excellent" if abs(bias_sigma) < 0.05 else "Good" if abs(bias_sigma) < 0.10 else "Poor"
                f.write(f"{'σ':<12} {est['sigma']:>12.4f} {true['sigma']:>12.4f} {bias_sigma:>+12.4f} {assess:>15}\n")
                
                # beta_p
                bias_bp = est["beta_p"] - true["beta_p"]
                assess = "Excellent" if abs(bias_bp) < 0.05 else "Good" if abs(bias_bp) < 0.10 else "Poor"
                f.write(f"{'β_p':<12} {est['beta_p']:>12.4f} {true['beta_p']:>12.4f} {bias_bp:>+12.4f} {assess:>15}\n")
                
                # beta_w
                bias_bw = est["beta_w"] - true["beta_w"]
                assess = "Excellent" if abs(bias_bw) < 0.05 else "Good" if abs(bias_bw) < 0.10 else "Poor"
                f.write(f"{'β_w':<12} {est['beta_w']:>12.4f} {true['beta_w']:>12.4f} {bias_bw:>+12.4f} {assess:>15}\n")
        ...
        if results["sparsity_analysis"]:
            f.write(f"\n{'=' * 80}\n")
            f.write(f"SPARSITY PATTERN RECOVERY\n")
            f.write(f"{'=' * 80}\n\n")
            
            sp = results["sparsity_analysis"]
            f.write(f"Signal Products (η ≠ 0):\n")
            f.write(f"  Average γ:  {sp['signal_gamma']:.4f}  (target: > 0.90)\n")
            f.write(f"  Assessment: {'✓ Excellent' if sp['signal_gamma'] > 0.90 else '~ Good' if sp['signal_gamma'] > 0.80 else '✗ Poor'}\n\n")
            
            f.write(f"Noise Products (η = 0):\n")
            f.write(f"  Average γ:  {sp['noise_gamma']:.4f}  (target: < 0.20)\n")
            f.write(f"  Assessment: {'✓ Excellent' if sp['noise_gamma'] < 0.20 else '~ Good' if sp['noise_gamma'] < 0.30 else '✗ Poor'}\n\n")
        ...
        if results.get("lu25_map_sparsity"):
            ...
            f.write(f"  Detected:    {spm['detected']}/{spm['total']} product-markets\n\n")
        ...
        for method_name in ["BLP + Cost IV", "BLP - Cost IV", "Shrinkage", "Lu25 MAP (No IV, No Inversion)"]:
            ...
            if est["status"] == "success":
                ...
                f.write(f"{method_name:<20} {bias_s:>+12.4f} {bias_p:>+12.4f} {overall:>15}\n")
    ...
    with open(filename, 'w') as f:
        ...
        for method_name in ["BLP + Cost IV", "BLP - Cost IV", "Shrinkage", "Lu25 MAP (No IV, No Inversion)"]:
            ...
            if est["status"] == "success":
                # sigma
                bias_s = est["sigma"] - true["sigma"]
                f.write(f"{method_name},sigma,{est['sigma']:.6f},{true['sigma']:.6f},{bias_s:+.6f},{abs(bias_s):.6f},success\n")
                
                # beta_p
                bias_p = est["beta_p"] - true["beta_p"]
                f.write(f"{method_name},beta_p,{est['beta_p']:.6f},{true['beta_p']:.6f},{bias_p:+.6f},{abs(bias_p):.6f},success\n")
                
                # beta_w
                bias_w = est["beta_w"] - true["beta_w"]
                f.write(f"{method_name},beta_w,{est['beta_w']:.6f},{true['beta_w']:.6f},{bias_w:+.6f},{abs(bias_w):.6f},success\n")
            else:
...
def main():
    ...
    try:
        ...
        print(f"  {'-'*55}")
        print(f"  {'σ':<12} {sigma_cost:>12.4f} {cfg.sigma_star:>12.4f} {format_bias(sigma_cost - cfg.sigma_star):>15}")
        print(f"  {'β_p':<12} {beta_cost[1]:>12.4f} {cfg.beta_p_star:>12.4f} {format_bias(beta_cost[1] - cfg.beta_p_star):>15}")
        print(f"  {'β_w':<12} {beta_cost[2]:>12.4f} {cfg.beta_w_star:>12.4f} {format_bias(beta_cost[2] - cfg.beta_w_star):>15}")
    ...
    try:
        ...
        print(f"\n  {'Parameter':<12} {'Estimate':>12} {'True':>12} {'Bias':>15}")
        print(f"  {'-'*55}")
        print(f"  {'σ':<12} {sigma_nocost:>12.4f} {cfg.sigma_star:>12.4f} {format_bias(sigma_nocost - cfg.sigma_star):>15}")
        print(f"  {'β_p':<12} {beta_nocost[1]:>12.4f} {cfg.beta_p_star:>12.4f} {format_bias(beta_nocost[1] - cfg.beta_p_star):>15}")
        print(f"  {'β_w':<12} {beta_nocost[2]:>12.4f} {cfg.beta_w_star:>12.4f} {format_bias(beta_nocost[2] - cfg.beta_w_star):>15}")
    ...
    try:
        ...
        print(f"\n  {'Parameter':<12} {'Estimate':>12} {'True':>12} {'Bias':>15}")
        print(f"  {'-'*55}")
        print(f"  {'σ':<12} {sigma_shrink:>12.4f} {cfg.sigma_star:>12.4f} {format_bias(sigma_shrink - cfg.sigma_star):>15}")
        print(f"  {'β_p':<12} {beta_shrink[1]:>12.4f} {cfg.beta_p_star:>12.4f} {format_bias(beta_shrink[1] - cfg.beta_p_star):>15}")
        print(f"  {'β_w':<12} {beta_shrink[2]:>12.4f} {cfg.beta_w_star:>12.4f} {format_bias(beta_shrink[2] - cfg.beta_w_star):>15}")
        
        print(f"\n  Sparsity Recovery:")
        print(f"    Signal (η≠0): γ̄ = {signal_gamma:.4f}  {'✓' if signal_gamma > 0.85 else '✗'}")
        print(f"    Noise (η=0):  γ̄ = {noise_gamma:.4f}  {'✓' if noise_gamma < 0.30 else '✗'}")
    ...
    # ===================================
    print_section("Step 5: Lu & Shimizu (2025)–style MAP estimator (Likelihood-based, sparse shocks, no inversion)")
    ...
    try:
        ...
        print(f"\n  {'Parameter':<12} {'Estimate':>12} {'True':>12} {'Bias':>15}")
        print(f"  {'-'*55}")
        print(f"  {'σ':<12} {sigma_lu:>12.4f} {cfg.sigma_star:>12.4f} {format_bias(sigma_lu - cfg.sigma_star):>15}")
        print(f"  {'β_p':<12} {beta_lu[1]:>12.4f} {cfg.beta_p_star:>12.4f} {format_bias(beta_lu[1] - cfg.beta_p_star):>15}")
        print(f"  {'β_w':<12} {beta_lu[2]:>12.4f} {cfg.beta_w_star:>12.4f} {format_bias(beta_lu[2] - cfg.beta_w_star):>15}")
    ...
    successful = sum(1 for est in results["estimates"].values() if est["status"] == "success")
        ...
        for method, est in results["estimates"].items():
            if est["status"] == "success":
                ...
                if method == "BLP + Cost IV" and bias_sigma > 0.10:
                    all_good = False
                elif method in ["Shrinkage", "Lu25 MAP (No IV, No Inversion)"] and bias_sigma > 0.15:
                    all_good = False
