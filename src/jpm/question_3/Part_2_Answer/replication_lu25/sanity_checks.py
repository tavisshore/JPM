...
def main():
    ...
    for j in range(J):
        ...
        print(f"{j:8d} {markets[0]['eta'][j]:8.1f} {detection_rate[j]:16.3f} {expected:>12}{marker}")
    ...
    for j in range(J):
        ...
        print(f"{j:8d} {markets[0]['eta'][j]:8.1f} {avg_abs_resid[j]:14.4f} {std_resid[j]:14.4f}{marker}")
    ...
    for name, (value, threshold, direction) in criteria.items():
        if direction == ">":
            ...
            status = "✓" if value < threshold else "✗"
        print(f"  {status} {name:<35}: {value:6.3f} (threshold: {direction}{threshold})")
