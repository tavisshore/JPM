...
path = "/scratch/projects/JPM/"
os.chdir(path)


# Import Data
df_is_stocks = pd.read_csv("assets/question_1/us_stocks.csv")
df_ratings = pd.read_csv("assets/question_1/ratings-history.csv")
...
rating_set = {"A", "AA", "AAA", "B", "BB", "BBB", "C", "CC", "CCC", "D"}
...
df_company_ratios = pd.DataFrame.from_dict(
    get_jsonparsed_data(
        ...
        + Symbol_list[0]
        + "&apikey=a0nD5Qm7EGwKlffrKuTlHorGFXJjlZrf"
    )
)
...
df_ratings_NA = df_ratings_NA.drop(
    [
        "Name",
        "Year",
        "date_y",
...
df_ratings_NA.to_csv(r"assets/rating_dataframe.csv", index=False, header=True)
